...
function insertionCoverage(state: EditorState, authorId: string): { total: number; authoredInsertion: number; deletion: number } {
    ...
    state.doc.descendants((node) => {
        ...
        if (node.marks.some((m) => m.type.name === 'insertion' && m.attrs.authorId === authorId)) {
            authoredInsertion += size;
        }
    });
...
function stripInsertionFlood(state: EditorState, insertionType: MarkType, authorId: string) {
    const coverage = insertionCoverage(state, authorId);
    if (coverage.total < 20) return null;
    if (coverage.deletion > 0) return null;
    if (coverage.authoredInsertion / coverage.total < 0.9) return null;

    const tr = state.tr;
    state.doc.nodesBetween(0, state.doc.content.size, (node, pos) => {
        if (!node.isText) return;
        const hasAuthoredInsertion = node.marks.some(
            (m) => m.type === insertionType && m.attrs.authorId === authorId,
        );
        if (hasAuthoredInsertion) {
